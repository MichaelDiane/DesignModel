 ...
 * jdk动态代理与cglib动态代理对比:
     ...
     *3.jdk 生成代理逻辑简单,执行效率要不cglib低,每次均需要反射动态调用;而cglib动态代理得生成逻辑更复杂,调用效率更高.生了一个包含了所有逻辑的FastClass[其根源是在于在fastclass中为代理对象和被代理对象均生成了有个序index,当想调用哪个的时候直接根据序直接定位调用],不再需要反射调用
     *
 */
